...
# ~/.gitconfig is deliberately NOT a symlink into this repo. `git config --global`
# (and anything that calls it, e.g. a self-hosted CI runner doing `--add
# safe.directory`) writes to ~/.gitconfig; when that path is a symlink, the write
# lands in the tracked file and machine-local state leaks into this public repo.
# Instead we generate a small real file that includes the tracked config, so global
# writes accumulate in an untracked file. Later includes win, so ~/.gitconfig.local
# still overrides the repo defaults.
setup_gitconfig() {
    local dst="$HOME/.gitconfig"
    local tracked="$DOTFILES_DIR/gitconfig"

    if [[ "$DRY_RUN" == true ]]; then
        echo "  [dry-run] write include-only $dst (includes $tracked, ~/.gitconfig.local)"
        return
    fi

    # Idempotent: leave an already-correct file alone so accumulated
    # `git config --global` settings in it survive a re-run of install.sh.
    if [[ -f "$dst" && ! -L "$dst" ]] \
        && grep -qxF "	path = $tracked" "$dst" \
        && grep -qxF "	path = ~/.gitconfig.local" "$dst"; then
        SKIPPED+=("$dst (already an include stub)")
        return
    fi

    if [[ -L "$dst" ]]; then
        # A symlink into this repo carries no unique state - the tracked file is
        # still included below - so drop it rather than leaving a stale .bak.
        rm "$dst"
        echo "  replaced symlink: $dst (now a real include-only file)"
    elif [[ -e "$dst" ]]; then
        mv "$dst" "${dst}.bak"
        echo "  backed up: $dst -> ${dst}.bak"
    fi
    ...
    cat > "$dst" <<EOF
# Generated by $DOTFILES_DIR/install.sh - not tracked by the dotfiles repo.
# Shared config lives in $tracked; identity and machine-local settings in
# ~/.gitconfig.local. Anything \`git config --global\` writes is appended below,
# which is the point: it stays out of the public dotfiles repo.
[include]
	path = $tracked
[include]
	path = ~/.gitconfig.local
EOF
    DONE+=("write include-only ~/.gitconfig")
}
